...
func TestService_Execute_ValidationFailure(t *testing.T) {
	svc := NewService(nil, nil, NewValidator(nil), nil)
...
func TestService_Execute_Success(t *testing.T) {
	...
	auditor := &stubAuditor{}
	svc := NewService(provider, auditor, NewValidator(provider), nil)
...
func TestService_Execute_ProviderFailure(t *testing.T) {
	...
	svc := NewService(provider, NewAuditor(nil), NewValidator(provider), nil)
