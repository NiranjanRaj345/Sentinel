...
func TestScheduler_Snapshot_BeforeStart_ReturnsError(t *testing.T) {
	s := New(5*time.Second, logger.New(logger.Info, io.Discard), nil, nil)
...
func TestScheduler_Start_InitialCollectionSucceeds(t *testing.T) {
	...
	s := New(100*time.Millisecond, logger.New(logger.Info, io.Discard), store, nil)
...
func TestScheduler_Snapshot_UpdatesOverTime(t *testing.T) {
	...
	s := New(50*time.Millisecond, logger.New(logger.Info, io.Discard), store, nil)
